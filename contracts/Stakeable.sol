...
contract Stakeable is ERC20, IStakeable {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        StakingSummary memory userSummary_ = getStakeSummary();
    ...
    function stake(uint256 amount) public override {
        ...
        uint256 reward_ = userSummary_.stakeAmount > 0
            ? newClaimableReward(userSummary_)
            : 0;
    ...
    function newClaimableReward(StakingSummary memory userSummary_)
        private
        view
        returns (uint256)
    {
    ...
    function claim() public override {
        StakingSummary memory userSummary_ = getStakeSummary();
        (bool success_, uint256 reward_) = newClaimableReward(userSummary_)
            .tryAdd(userSummary_.reward);
        require(success_, "Stakeable: MATH ERROR");
